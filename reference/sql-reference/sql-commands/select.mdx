...
* `[ WITH ...] String` \<Optional> Defines a common table expression (CTE), which is a named subquery. For more information, read [WITH](/reference/sql-reference/sql-commands/with).
...
* \{ PIVOT | UNPIVOT } ( \<expression> ) `String` `PIVOT `\<Optional> converts a set of data from rows into columns. `UNPIVOT` converts a set of data from columns into rows. The expression can be one of the following:
  ...
  * pivot\_in\_clause: Filters the values for the columns *pivot\_for\_clause*. Each of the values in this clause will be a separate column.
