---
title: "eth.eigenlayer.strategy_manager_withdrawal_completed"
---
...
The `staker` who queued the withdrawal from EigenLayer.
...
The party specified by `staker` who completed the queued withdrawal and received the withdrawn funds.
...
The contract address of the strategy that has a completed withdrawal.
...
The number of shares withdrawn as a hexadecimal string.

`caller`

CHARACTER VARYING

The address of the caller on the StrategyManager contract

`token`

CHARACTER VARYING

The token that `depositor` deposited.
...
The number of tokens deposited as a hexadecimal string.

`receive_as_tokens`

BOOLEAN

Whether the withdrawer has flagged to receive the funds as tokens
